    ...
    def print_errors(self):
        ...
        if self.errors:
            for bookstore, errors in self.errors.iteritems():
                print unicode(bookstore)
                for error in errors:
                    print u'%s (id: %s), cena w bazie: %s, cena na stronie %s' % (error['book'].url.encode('utf8'), error['book'].id, error['book'].price, error['page_price'])
        else:
            print u'brak'
        ...
        if self.invalid_links:
            for bookstore, items in self.invalid_links.iteritems():
                print unicode(bookstore)
                for item in items:
                    print u'(id: %s) %s, powod: %s' % (item['book'].id, item['book'].url.encode('utf8'), item['reason'])
        else:
            print u'brak'
