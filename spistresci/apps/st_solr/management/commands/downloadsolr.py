...
class Command(BaseCommand):
    ...
    def handle(self, version=SOLR_VERSION, *args, **options):
        ...
        if version not in SOLR_VERSIONS:
            print "Please define %s SOLR version in %s" % (
                version,
                os.path.realpath(__file__),
            )
            return

        solr_archive_file = os.path.join(
            DOWNLOAD_DIR,
            "solr-%s.zip" % version,
        )

        if not os.path.isfile(solr_archive_file):
            url = SOLR_VERSIONS[version].get(
                'url',
                SOLR_DEFAULT_DOWNLOAD_URL % {'version': version},
            )
            downloadFile(url, solr_archive_file)

        calculate_hash = hashlib.md5(
            open(solr_archive_file).read()
        ).hexdigest()

        if calculate_hash != SOLR_VERSIONS[version]['md5']:
    ...
    def unpackSolr(self, file, dir, version):
        ...
        unpackZIP(file, dir)
        shutil.move(
            os.path.join(dir, 'solr-%s' % version),
            os.path.join(dir, '..', '_solr/'),
        )
        shutil.rmtree(dir)

        shutil.move(
            os.path.abspath(os.path.join(dir, '..', '_solr/')),
            dir
        )
